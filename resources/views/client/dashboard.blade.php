...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-blue-600 to-cyan-600 rounded-lg shadow-lg p-6 text-white">
        <h2 class="text-2xl font-bold">Welcome back, {{ $client->name }}!</h2>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-3xl font-bold text-gray-900">{{ $projects->count() }}</p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-3xl font-bold text-green-600">{{ $currency }}{{ number_format($totalPaid, 2) }}</p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-3xl font-bold text-orange-600">{{ $currency }}{{ number_format($totalPending, 2) }}</p>
                </div>
    ...
    <div class="bg-white rounded-lg shadow">
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex items-center justify-between">
                ...
                <a href="{{ route('client.projects') }}" class="text-sm text-blue-600 hover:text-blue-700 font-medium">View All</a>
            </div>
        </div>
        <div class="p-6">
            @if($projects->isEmpty())
                    ...
                    @foreach($projects->take(5) as $project)
                        <div class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition">
                            <div class="flex items-start justify-between">
                                <div class="flex-1">
                                    ...
                                    <div class="mt-3">
                                        <div class="flex items-center justify-between text-sm text-gray-600 mb-1">
                                            ...
                                            <span class="font-medium">{{ $currency }}{{ number_format($project->amount_paid, 2) }} / {{ $currency }}{{ number_format($project->budget ?? $project->tasks->sum('price'), 2) }}</span>
                                        </div>
                                ...
                                <a href="{{ route('client.project', $project->id) }}" class="ml-4 inline-flex items-center px-3 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
                                    View Details
                                </a>
                            </div>
                        </div>
        ...
        <div class="bg-white rounded-lg shadow">
            <div class="px-6 py-4 border-b border-gray-200">
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('client.payments') }}" class="text-sm text-blue-600 hover:text-blue-700 font-medium">View All</a>
                </div>
            </div>
            <div class="p-6">
                @if($payments->isEmpty())
                    ...
                    <div class="space-y-3">
                        @foreach($payments->take(5) as $payment)
                            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                <div>
                                    <p class="text-sm font-medium text-gray-900">{{ $payment->project->title }}</p>
                                    <p class="text-xs text-gray-500">{{ $payment->date->format('M d, Y') }}</p>
                                </div>
                                <div class="text-right">
                                    <p class="text-sm font-semibold text-green-600">{{ $currency }}{{ number_format($payment->amount, 2) }}</p>
                                    <p class="text-xs text-gray-500">{{ ucfirst($payment->type) }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
        ...
        <div class="bg-white rounded-lg shadow">
            <div class="px-6 py-4 border-b border-gray-200">
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('client.tasks') }}" class="text-sm text-blue-600 hover:text-blue-700 font-medium">View All</a>
                </div>
            </div>
            <div class="p-6">
                @if($tasks->isEmpty())
