...
@section('title', 'Tasks - Client Portal')
    ...
    @else
        <div class="space-y-3">
            ...
            @foreach($statusOrder as $status)
                @if(isset($groupedTasks[$status]) && $groupedTasks[$status]->count() > 0)
                    <div class="bg-white rounded-lg shadow">
                        <div class="px-6 py-4 border-b border-gray-200">
                            <div class="flex items-center justify-between">
                                <h3 class="text-lg font-semibold text-gray-900">
                                    {{ ucfirst(str_replace('_', ' ', $status)) }}
                                    <span class="ml-2 text-sm text-gray-500">({{ $groupedTasks[$status]->count() }})</span>
                                </h3>
                            </div>
                        </div>
                                    ...
                                    <div class="flex items-start justify-between">
                                        <div class="flex-1">
                                            ...
                                            <div class="flex flex-wrap items-center gap-4 text-sm text-gray-500">
                                                @if($task->project)
                                                    <a href="{{ route('client.project', $task->project_id) }}" class="inline-flex items-center text-blue-600 hover:text-blue-700">
                                                ...
                                                @if($task->deadline)
                                                    <span class="inline-flex items-center {{ $task->is_overdue ? 'text-red-600' : '' }}">
                                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-4 h-4 mr-1">
                                                ...
                                                @if($task->price)
                                                    <span class="inline-flex items-center">
                                                        ...
                                                        {{ $currency }}{{ number_format($task->price, 2) }}
                                                    </span>
                                                @endif
                                            </div>
                                        </div>
