...
@section('page-title', 'Revenue Categories')

@section('content')
<div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
  <div>
    <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-200">Revenue Categories</h2>
    <p class="text-sm text-gray-500 dark:text-gray-400">Total: {{ $totalCategories }}</p>
  </div>
  <a href="{{ route('revenue-categories.create') }}" class="px-4 py-2 bg-primary-600 text-white rounded hover:bg-primary-700 whitespace-nowrap text-center">Add Category</a>
</div>
        ...
        @forelse($categories as $category)
          <tr>
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm">
              <form action="{{ route('revenue-categories.destroy', $category) }}" method="POST" onsubmit="return confirm('Delete this category?')">
                @csrf
                @method('DELETE')
    ...
    @forelse($categories as $category)
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
            <div class="flex items-start justify-between mb-2">
                ...
                <form action="{{ route('revenue-categories.destroy', $category) }}" method="POST" onsubmit="return confirm('Delete this category?')" class="ml-2">
                    @csrf
                    @method('DELETE')
