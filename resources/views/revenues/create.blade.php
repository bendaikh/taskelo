...
@section('page-title', 'Add Revenue')

@section('content')
<div class="max-w-2xl">
  <form action="{{ route('revenues.store') }}" method="POST" class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 space-y-4">
    @csrf
    ...
    <div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        <select name="revenue_category_id" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100">
          <option value="">-- Select Category --</option>
          @foreach($categories as $cat)
            <option value="{{ $cat->id }}" {{ (string) old('revenue_category_id') === (string) $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
          @endforeach
        </select>
    ...
    <div class="flex items-center justify-end gap-3">
      <a href="{{ route('revenues.index') }}" class="px-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded">Cancel</a>
      <button class="px-4 py-2 bg-primary-600 text-white rounded hover:bg-primary-700">Save Revenue</button>
    </div>
  </form>
</div>
@endsection
